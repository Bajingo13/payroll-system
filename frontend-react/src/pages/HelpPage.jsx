...
const QUICKSTART = [
  { step: '01', title: 'Set Up Company Info', desc: 'Go to System Tools -> Company Settings to fill in your company profile, upload your logo, and define policies.', icon: 'building' },
  { step: '02', title: 'Add Employees', desc: 'Go to HRIS -> Employee File and add employees with personal details, employment info, and government IDs.', icon: 'user' },
  { step: '03', title: 'Assign Schedules', desc: 'Use Schedule Management to create work schedules and assign them to employees before running payroll.', icon: 'calendar' },
  { step: '04', title: 'Run Payroll', desc: 'Go to Payroll Computation, select the period and group, then click Generate to compute payroll.', icon: 'wallet' },
];

const MODULES = [
  { icon: 'users', name: 'Employee File', desc: 'Complete profiles, government IDs, salary, and 201 documents.' },
  { icon: 'time', name: 'Attendance', desc: 'Daily timekeeping, overtime tracking, and schedule management.' },
...
const FAQS = [
  ...
  { q: 'What if I miss approving leave before payroll?', a: 'Unapproved leave will not deduct from balance. You can manually process the deduction in the next payroll period.' },
];

const ISSUES = [
  { icon: 'alert', problem: 'Server error on Generate', fix: 'Make sure the server is running and the payroll run exists. Refresh and try again.' },
  { icon: 'clipboard', problem: 'No rows in reports', fix: 'Check the month/year or confirm employees are included in the payroll run for that period.' },
  { icon: 'download', problem: 'Export does not download', fix: 'Hard refresh (Ctrl+F5) or use a different browser. Disable pop-up blockers for this site.' },
  { icon: 'key', problem: 'Cannot log in', fix: 'Check your credentials. Ask your admin to reset your password under User Settings.' },
  { icon: 'document', problem: 'Wrong deductions on payslip', fix: 'Verify BIR tax brackets and contribution tables in System Configuration are correct.' },
];

function IconBox({ name, color = '#1e40af', bg = '#eff6ff', size = 18 }) {
  return (
    <span style={{ width: 34, height: 34, borderRadius: 10, background: bg, color, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
  ...
  return (
    <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 0 48px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: 16 }}>
          {QUICKSTART.map(q => (
            <div key={q.step} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', position: 'relative' }}>
              ...
              <div style={{ marginBottom: 8 }}><IconBox name={q.icon} /></div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
          {MODULES.map(m => (
            <div key={m.name} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 14, padding: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              <div style={{ marginBottom: 8 }}><IconBox name={m.icon} /></div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {FAQS.map((f, i) => (
            <div key={f.q} style={{ background: '#fff', border: `1.5px solid ${openFaq === i ? '#3b82f6' : '#e2e8f0'}`, borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', transition: 'border-color .2s' }}>
              <button
