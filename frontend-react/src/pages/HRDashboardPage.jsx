  ...
  return (
    <>
      <header className="dboard-hero">
        <div>
          <p className="dboard-hero-kicker">HR Dashboard</p>
          <h2>Welcome back, {user?.full_name?.split(' ')[0] || 'HR Manager'}</h2>
          <p className="dboard-hero-sub">People operations overview for today.</p>
        </div>
      ...
      <div className="dboard-kpi-row">
        {[
          { icon: 'users', label: 'Total Employees', value: Number(summary.totalEmployees || 0).toLocaleString(), sub: `${activeP}% active`, bg: '#e0f2fe' },
          { icon: 'check', label: 'Active', value: Number(activeEmp || 0).toLocaleString(), sub: 'Currently employed', bg: '#dcfce7' },
          { icon: 'wallet', label: 'Payroll Runs', value: Number(summary.processedPayrolls || 0).toLocaleString(), sub: 'Processed', bg: '#ede9fe' },
          { icon: 'clipboard', label: 'Pending Leaves', value: pendingLeaves, sub: 'Awaiting approval', bg: '#fef9c3' },
          { icon: 'alert', label: 'Turnover Risk', value: as.highTurnoverRisks || 0, sub: 'High-risk flags', bg: '#fee2e2' },
        ].map((s, i) => (
