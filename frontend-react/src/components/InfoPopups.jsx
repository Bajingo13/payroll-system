  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        <MiniCard icon="👤" tag="HRIS" title="Employee Records" desc="Complete profiles, government IDs, 201 files, and org setup." accentBg="#eff6ff" />
        <MiniCard icon="💰" tag="Payroll" title="Computation & Processing" desc="Automated payroll with BIR, SSS, PhilHealth, and Pag-IBIG." accentBg="#f0fdf4" accent="#15803d" />
        <MiniCard icon="📊" tag="Reports" title="Compliance Reports" desc="Payslips, payroll journals, government reports, and auditing." accentBg="#fef3c7" accent="#d97706" />
        <MiniCard icon="📱" tag="Mobile" title="HRIS Mobile App" desc="Employees can clock in/out, file requests, and view payslips." accentBg="#f5f3ff" accent="#7c3aed" />
      </div>
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 16 }}>
        {[
          ['🏢', '01', 'Company Setup', 'System Tools → Company Settings → fill in your profile.'],
          ['👤', '02', 'Add Employees', 'HRIS → Employee File → add profiles and government IDs.'],
          ['📅', '03', 'Assign Schedules', 'Schedule Management → assign work schedules.'],
          ['💰', '04', 'Run Payroll', 'Payroll Computation → select period → Generate.'],
          ...
          <div key={step} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 12, padding: '12px 14px', display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            <div style={{ width: 28, height: 28, borderRadius: 8, background: '#eff6ff', color: '#1e40af', fontSize: 10, fontWeight: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{step}</div>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: '#0f172a', marginBottom: 2 }}>{icon} {title}</div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {[
          ['⚠️', 'Server error on Generate', 'Confirm the server is running and your payroll run exists.'],
          ['📋', 'No records returned', 'Try a different period/month/year with generated payroll.'],
          ['💾', 'Export not downloading', 'Hard refresh (Ctrl+F5) or try a different browser.'],
          ['🔑', 'Cannot log in', 'Ask your admin to reset your password under User Settings.'],
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: 16, overflow: 'hidden' }}>
        {[
          { icon: '✉️', color: '#eff6ff', iconColor: '#1e40af', label: 'Email', value: 'astreablueintelligenceinc@gmail.com', href: 'mailto:astreablueintelligenceinc@gmail.com' },
          { icon: '📍', color: '#fef3c7', iconColor: '#d97706', label: 'Address', value: '20F Unit 2004, Philippine AXA Life Centre, 1286 Sen. Gil Puyat Ave., Makati City' },
          { icon: '🕐', color: '#f0fdf4', iconColor: '#15803d', label: 'Hours', value: 'Mon – Fri · 8:00 AM – 5:00 PM (PST)' },
        ].map((item, i) => (
          <div key={item.label} style={{
            display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px',
            borderTop: i > 0 ? '1px solid #f1f5f9' : 'none',
          }}>
            <div style={{ width: 36, height: 36, borderRadius: 10, background: item.color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0 }}>
              {item.icon}
            </div>
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
          {[
            { icon: '💼', label: 'Payroll', sub: 'HR / Payroll Team', color: '#eff6ff' },
            { icon: '📅', label: 'Leave & OT', sub: 'HR Department', color: '#f0fdf4' },
            { icon: '🔧', label: 'System', sub: 'Sys. Admin', color: '#fef3c7' },
          ].map(d => (
